...
Esto levanta cuatro servicios: `postgres`, `backend`, `frontend` (solo compila los estáticos, no sirve nada por sí mismo) y `nginx`, que es el único que necesita ser alcanzable desde otros equipos — sirve el frontend compilado y reenvía `/api/` al backend interno.

:::warning
`docker-compose.yml` también publica el puerto `8080` del backend para depurar en directo. En un despliegue real conviene cerrarlo en el firewall del servidor: todo el tráfico de la app debe pasar por `nginx` en el puerto 80.
:::
